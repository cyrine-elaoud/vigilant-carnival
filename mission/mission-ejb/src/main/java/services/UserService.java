...
public class UserService extends GenericDAO<User> implements UserServiceRemote, UserServiceLocal {
	...
	@Override
	public List<Publicity> findPublicitiesByMember(int id) {
		return entityManager.createQuery("SELECT p FROM Publicity p WHERE p.member.id=:idMember ", Publicity.class)
				.setParameter("idMember", id).getResultList();
	}
